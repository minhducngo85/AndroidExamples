 ...
 */
package com.berry.mobile.piggybank.database;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // asset column
        sql += COL_ASSET_ID + " INTEGER, ";
        // search column
        sql += COL_SEARCH + " VARCHAR(300))";
        db.execSQL(sql);

        /** creates asset table */
        sql = "CREATE TABLE IF NOT EXISTS " + TABLE_ASSET + " (";
        // id column
        sql += COL_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, ";
        // name column
        ...
        sql = "INSERT INTO " + TABLE_ASSET + " VALUES(1,'Cash')";
        db.execSQL(sql);
        sql = "INSERT INTO " + TABLE_ASSET + " VALUES(2,'Bank Account')";
        db.execSQL(sql);
        sql = "INSERT INTO " + TABLE_ASSET + " VALUES(3,'Investments')";
        db.execSQL(sql);
        sql = "INSERT INTO " + TABLE_ASSET + " VALUES(4,'Credit')";
        db.execSQL(sql);
        sql = "INSERT INTO " + TABLE_ASSET + " VALUES(5,'Other')";
        db.execSQL(sql);
     ...
     */
    public List<Asset> convertCursorToAssets(Cursor cursor) {
    ...
    public List<Transaction> convertCursorToTransaction(Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                trans.setAssetId(cursor.getInt(cursor.getColumnIndex(COL_ASSET_ID)));
    ...
    public Cursor searchTransactions(String searchString) {
        ...
        if (searchString == null || searchString.trim().isEmpty()) {
            return db.rawQuery("SELECT * FROM " + TABLE_TRANSACTION + " ORDER BY " + COL_DATE, null);
        }
        String sql = "SELECT * FROM " + TABLE_TRANSACTION + " WHERE " + COL_SEARCH + " LIKE ?" + " ORDER BY " + COL_DATE;
    ...
    public Cursor getAssets() {
        SQLiteDatabase db = this.getReadableDatabase();
        return db.rawQuery("SELECT * FROM " + TABLE_ASSET, null);
    }
    ...
    public void saveTransaction(Transaction item) {
        ...
        values.put(COL_ASSET_ID, item.getAssetId());
