package com.android.tuto.ch9.intentservice.util;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Cursor searchQuakesAsRawData(String searchString, int minMag) {
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT * FROM " + TABLE_EARTHQUAKE + " WHERE " + COL_SUMMARY_3 + " LIKE '%" + searchString + "%' AND " + COL_MAGNITUDE_6
                + " >= " + minMag + " ORDER BY " + COL_DATE_1 + " DESC";
        Cursor cursor = db.rawQuery(query, null);
        return cursor;
    }
